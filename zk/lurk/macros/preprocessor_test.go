...
func TestPreProcessValidParentheses(t *testing.T) {
	...
	tests := []testVector{
		...
		{"!(param nullifiers 0)", "(nth 0 (car (cdr public-params)))"},
		{"!(param nullifiers 1)", "(nth 1 (car (cdr public-params)))"},
		...
		{"!(param priv-out 3)", "(nth 3 (cdr private-params))"},
		{"!(param priv-out 4)", "(nth 4 (cdr private-params))"},
		{"!(param priv-in 2 amount)", "(car (nth 2 (car private-params)))"},
		{"!(param priv-in 2 asset-id)", "(car (cdr (nth 2 (car private-params))))"},
		{"!(param priv-in 2 salt)", "(car (cdr (cdr (nth 2 (car private-params)))))"},
		{"!(param priv-in 2 state)", "(car (cdr (cdr (cdr (nth 2 (car private-params))))))"},
		{"!(param priv-in 2 commitment-index)", "(car (cdr (cdr (cdr (cdr (nth 2 (car private-params)))))))"},
		{"!(param priv-in 2 inclusion-proof)", "(car (cdr (cdr (cdr (cdr (cdr (nth 2 (car private-params))))))))"},
		{"!(param priv-in 2 locking-params)", "(car (cdr (cdr (cdr (cdr (cdr (cdr (cdr (nth 2 (car private-params))))))))))"},
		{"!(param priv-in 2 unlocking-params)", "(car (cdr (cdr (cdr (cdr (cdr (cdr (cdr (cdr (nth 2 (car private-params)))))))))))"},
		{"!(param priv-out 3 script-hash)", "(car (nth 3 (cdr private-params)))"},
		{"!(param priv-out 3 amount)", "(car (cdr (nth 3 (cdr private-params))))"},
