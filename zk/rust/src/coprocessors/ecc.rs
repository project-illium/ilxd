        ...
        F: PrimeField,
{
    ...
    pub fn alloc<CS: ConstraintSystem<F>>(
        mut cs: CS,
        coords: Option<(F, F, bool)>,
    ) -> Result<Self, SynthesisError> {
        let x = AllocatedNum::alloc(cs.namespace(|| "x"), || {
            Ok(coords.map_or(F::ZERO, |c| c.0))
        })?;
        let y = AllocatedNum::alloc(cs.namespace(|| "y"), || {
            Ok(coords.map_or(F::ZERO, |c| c.1))
        })?;
    ...
    pub fn default<CS: ConstraintSystem<F>>(mut cs: CS) -> Result<Self, SynthesisError> {
    ...
    pub fn double<CS: ConstraintSystem<F>>(&self, mut cs: CS) -> Result<Self, SynthesisError> {
        //*************************************************************/
        // lambda = (F::from(3) * self.x * self.x + E::GE::A())
        //  * (F::from(2)) * self.y).invert().unwrap();
        /*************************************************************/

        // Compute tmp = (F::ONE + F::ONE)* self.y ? self != inf : 1
        ...
        // Now compute lambda as (F::from(3) * self.x * self.x + E::GE::A()) * tmp_inv
    ...
    pub fn select_point_or_infinity<CS: ConstraintSystem<F>>(
        mut cs: CS,
        a: &Self,
        condition: &Boolean,
    ) -> Result<Self, SynthesisError> {
...
pub struct AllocatedPointNonInfinity<F: PrimeField> {
    x: AllocatedNum<F>,
    y: AllocatedNum<F>,
}

impl<F> AllocatedPointNonInfinity<F>
    where
        F: PrimeField,
{
    ...
    pub fn from_allocated_point(p: &AllocatedPoint<F>) -> Self {
        Self {
            x: p.x.clone(),
            y: p.y.clone(),
        }
