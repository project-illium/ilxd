...
func TestHashToPoint(t *testing.T) {
	for _, test := range testVectors {
		...
		x, y := hashToPoint(secp.S256(), data)
		fmt.Println(hex.EncodeToString(x.Bytes()), hex.EncodeToString(y.Bytes()))
		fmt.Println(test.point[0], test.point[1])
		if hex.EncodeToString(x.Bytes()) != test.point[0] || hex.EncodeToString(y.Bytes()) != test.point[1] {
			t.Error("hashToPoint return incorrect point")
		}
	}
}
...
func BenchmarkMultiset_AddRemove(b *testing.B) {
	for i := 0; i < b.N; i++ {
		m := NewMultiset(secp.S256())
		for _, test := range testVectors {
			data, err := hex.DecodeString(test.dataElementHex)
